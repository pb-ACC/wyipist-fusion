...
class TerminalFlash extends CI_Controller
{
        ...
        if($this->session->userdata('logged_in')) {
            ...
            switch ($emp) {
                case 'Ceragni':
                    ...
                    $st_exp = '\'ST012\'';
                    $cond = " AND isnull(RefCeragni,0)=1 ";
                    $this->recolhe_dados($st_arm, $st_prod, $st_ver, $st_exp, $cond);
                    break;
                case 'Certeca':
                    $st_arm = '\'CL001\', \'FB003\'';                       
                    $st_prod = '\'FB001\''; 
                    $st_ver = '\'YYY\'';
                    $st_exp = '\'CL006\', \'FB006\'';      
                    $cond = " AND isnull(RefCerteca,0)=1 ";
                    $this->recolhe_dados($st_arm, $st_prod, $st_ver, $st_exp, $cond);
                    break;
                default:
                    # code...
                    ...
                    $cond = " AND isnull(RefCeragni,0)=1 AND isnull(RefCerteca,0)=1";
                    $this->recolhe_dados($st_arm, $st_prod, $st_ver, $st_exp, $cond);  
                    break;
            }
    ...
    function recolhe_dados($st_arm, $st_prod, $st_ver, $st_exp, $cond){
        $this->load->helper('url');
        $this->load->library('session');
        $session_data = $this->session->userdata('logged_in');            
        ...
        if($this->session->userdata('logged_in')) {
              ...
              $result = $this->Recolhe_Dados->recolhe_dados($st_arm, $st_prod, $st_ver, $st_exp, $cond, $username, $funcionario_gpac);                      
